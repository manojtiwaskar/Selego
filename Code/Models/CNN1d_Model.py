...
'''This script is used to train CNN1d based time series forecasting model using Selego (ot other variate selection technique) selected variates.
...
if __name__ == "__main__":
    ...
    # specify the absolute file path of dataset
    ...
    for run in range(1, 2):
        ...
        # model configuration
        model = Sequential()
        model.add(Conv1D(200, kernel_size=3, strides=1, padding="same",
                         input_shape=(train_X.shape[1], train_X.shape[2])))
        model.add(Conv1D(1, kernel_size=3, strides=1, padding="same"))
